...
MessageElement::MessageElement(const char* player_name, char* msg,int col)
{
    ...
    if (strncmp(msg, bot_tag, 5)==0) {
        ...
        actual_msg = msg + 5;
        actual_col = 3;
    } else if ((strcmp(msg, "/start")==0||strcmp(msg, ".Î•‰™•")==0) && is_start==0) {
		name = (char*)"$";
		actual_msg = (char*)"ë‚†‡‚ Á•‡•ß 20 ·•™„≠§";
		if (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(), "mammoth hunt")==0 || strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"mamont")==0) 
			actual_msg = (char*)"ë‚†‡‚ ¨†¨Æ≠‚† Á•‡•ß 20 ·•™„≠§, ÆÂÆ‚≠®™Æ¢ Á•‡•ß 40";
		actual_col = 3;
		is_start = 1;
	} else if ((strcmp(msg, "/finish")==0||strcmp(msg, ".†Ë‚ËÎ‡")==0) && (is_start==2 || is_start==3)) {
		name = (char*)"$";
		actual_msg = (char*)"î®≠®Ë";
		actual_col = 3;
		is_start = 0;
	} 
	else if ((strcmp(msg, "Ô")==0||strcmp(msg, "z")==0 || strcmp(msg, "ü")==0||strcmp(msg, "Z")==0) && is_start==2 && whoIsKvach==1) {
		whoIsKvach = 2;
	...
	else if ((strcmp(msg, "/rekvach")==0||strcmp(msg, ".™„´¨‰·‡")==0) && is_start==2 && strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(), "mechokvach")==0) {
		name = (char*)"$";
		actual_msg = (char*)"ä‚Æ ™¢†Á? (Ô/z)";
		actual_col = 3;
		whoIsKvach=1;
		kvachTime=-1;
	}
	else if ((strcmp(msg, "/scancel")==0 || strcmp(msg, ".Î·‰‚·„§")==0) && is_start==1) {
		name = (char*)"$";
		actual_msg = (char*)"ë‚†‡‚ Æ‚¨•≠•≠";
		actual_col = 3;
		is_start = 0;
